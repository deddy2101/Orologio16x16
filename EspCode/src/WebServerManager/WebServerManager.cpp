...
void WebServerManager::initServer()
{
  ...
  server.on("/snow", HTTP_GET, [this](AsyncWebServerRequest *request)
            { request->send(200, "text/plain", String(settings->getUseSnow() ? "true" : "false")); });
  ...
  server.on("/setSnow", HTTP_POST, [this](AsyncWebServerRequest *request) {
    if (request->hasParam("snow", true)) {
      String snow = request->getParam("snow", true)->value();
      settings->setShowSnow(snow == "true");
      settings->saveSettings();
      request->send(200, "text/plain", "Snow updated");
    } else {
      request->send(400, "text/plain", "Missing parameters");
